...
async function executeTool(name: string, args: Record<string, unknown>): Promise<ToolResult> {
  switch (name) {
      case "gemini_chat": {
        const { message: rawMessage, message_file, conversation_id = "default", system_prompt, images = [], output_file, response_format, response_schema } = args as any;
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  try {
    if (name === "batch") {
      const { steps: rawSteps, operations } = args as any;

      // Normalize: flat operations array → single step
      const steps: Array<Array<{ tool: string; args?: Record<string, unknown> }>> =
        rawSteps ?? (operations ? [operations] : []);
      ...
      const allResults: Array<{ step: number; index: number; tool: string; success?: boolean; text: string; hasImages?: boolean }> = [];
